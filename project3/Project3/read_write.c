/*
CSE 531 - Project 3. Submission by Sumit Rawat (ASU ID - 1216225348) and Harsh Virani (ASU ID - 1215137742)
...
void reader_exit(int ID)
{
	...
	if (rc == 0 && wwc > 0) 
	{
	    	// if this was the last reader, and there are waiting writers, open 
...
void writer_exit(int ID)
{
	P(mutex);
	wc--;
	if (rwc > 0) {		// first let all the readers go.
	    V(r_sem);
	} else if (wwc > 0) {	// let another writer go
	    V(w_sem);
	} else {		// lastly give up the mutex
	    V(mutex);
	}
}
